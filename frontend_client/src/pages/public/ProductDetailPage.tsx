...
export default function ProductDetailPage() {
  ...
  const [item, setItem] = useState<Service | Product | null>(null);
  ...
  const handleAddToCart = () => {
    ...
    addItem({
      ...
      categoryName: item.category && typeof item.category === 'object' ? item.category.name : undefined,
    });
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 md:p-8">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-medium text-brand-600 bg-brand-50 px-2.5 py-1 rounded-full">{itemType === 'service' ? t('catalog.typeServices') : t('catalog.typeProducts')}</span>
          </div>
