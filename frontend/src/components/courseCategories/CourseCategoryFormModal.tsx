...
const emptyForm: CourseCategoryPayload = {
  name: '',
  description: '',
  color: '',
  icon: '',
...
interface CourseCategoryFormModalProps {
  isOpen: boolean;
  category: CourseCategory | null; // null = création
  onClose: () => void;
...
export function CourseCategoryFormModal({
  ...
  const [form, setForm] = useState<CourseCategoryPayload>(emptyForm);
  ...
  function update<K extends keyof CourseCategoryPayload>(field: K, value: string) {
    setForm((prev) => ({ ...prev, [field]: value }));
  }

  function buildPayload(): CourseCategoryPayload {
    const payload: CourseCategoryPayload = { name: form.name.trim() };
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      const saved = isEditing
        ? await courseCategoriesApi.update(category.id, payload)
        : await courseCategoriesApi.create(payload);

      showToast(
        isEditing ? t('courseCategories.updated') : t('courseCategories.saved'),
        'success'
      );
      onSaved(saved);
      onClose();
    } catch (error) {
      setFormError(extractErrorMessage(error, t('courseCategories.saveFailed')));
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditing ? t('courseCategories.editTitle') : t('courseCategories.createTitle')}
      maxWidth="max-w-2xl"
    >
