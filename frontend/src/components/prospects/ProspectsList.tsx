...
interface ProspectsListProps {
  /** Filtre optionnel par agence. Sans agence (ex: espace commercial), le backend
   * limite déjà automatiquement un commercial à ses propres prospects. */
  agencyId?: string;
  /** Masque le sélecteur de commercial dans le formulaire (ex: un commercial ne
   * peut créer des prospects que pour lui-même). */
  hideCommercialField?: boolean;
}

export default function ProspectsList({ agencyId, hideCommercialField = false }: ProspectsListProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setFormError(null);
    setFieldErrors({});
    if (!hideCommercialField && !form.commercial_id) {
      setFieldErrors({ commercial_id: t('prospects.commercialRequired') });
      setIsSubmitting(false);
      return;
    }
    const payload = hideCommercialField ? { ...form, commercial_id: undefined } : form;
    try {
      if (editing) {
        await prospectsApi.update(editing.id, payload);
        showToast(t('prospects.updated'), 'success');
      } else {
        await prospectsApi.create({ ...payload, agency_id: agencyId || undefined });
