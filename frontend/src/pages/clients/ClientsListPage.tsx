...
export default function ClientsListPage() {
  ...
  const fetchClients = useCallback(async (filters: { search?: string; page: number }) => {
    ...
    try {
      ...
      setLoadError(extractErrorMessage(error, 'Impossible de charger les clients.'));
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  async function toggleActive(client: UserListItem) {
    ...
    try {
      ...
      showToast(
        updated.is_active ? 'Client réactivé avec succès.' : 'Client désactivé avec succès.',
        'success'
      );
    } catch (error) {
      showToast(extractErrorMessage(error, 'Impossible de modifier le statut du client.'), 'error');
    } finally {
      setTogglingId(null);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Clients / Apprenants</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Comptes créés via la page d'inscription publique. Leur interface d'inscription aux services
          arrivera dans un prochain module.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-900">
        {isLoading ? (
          ...
          <p className="p-6 text-sm text-gray-500 dark:text-gray-400">
            Aucun client trouvé. Les clients s'inscrivent via la page d'inscription publique.
          </p>
        ) : (
                    ...
                    <td className="px-5 py-3">
                      <div className="flex justify-end gap-1.5">
                        ...
                        <Button
                          ...
                          title={c.is_active ? 'Désactiver le compte' : 'Réactiver le compte'}
                        >
                          {c.is_active ? (
          ...
          <dl className="flex flex-col gap-3 text-sm">
            ...
            <div>
              ...
              <dd className="text-gray-800 dark:text-gray-100">
                {viewClient.is_active ? 'Actif' : 'Inactif'}
              </dd>
            </div>
          </dl>
        )}
